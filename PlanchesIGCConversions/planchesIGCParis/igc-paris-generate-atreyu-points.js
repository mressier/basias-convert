...
const sourceFileFullPixelConfig = "./Resources/generate-points-config/planches-pixel-full-config.csv"
...
const pixelConfigs = pixelConfigObjects.map(pixel => new FullPixelConfig(pixel))
const atreyuConfig = pixelConfigs.filter(config => config.name == "Atreyu")

const filesConfig = boards.map(board =>
  new FileConfig({
    "folder": "Atreyu",
